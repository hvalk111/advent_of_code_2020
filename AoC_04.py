...
input_list2 = [i.replace("\n", " ") for i in input_list]

input_list3 = [i.split(" ") for i in input_list2]

list_of_dicts = []
for str_list in input_list3: 
    list_of_dicts.append({k.split(":")[0]:k.split(":")[1] for k in str_list})

test_group = ['iyr', 'hgt', 'byr', 'pid', 'eyr', 'hcl', 'ecl']
eye_color = ['amb','blu','brn','gry','grn','hzl','oth']
valid_count = 0
for i in list_of_dicts:
    if sum(elem in test_group for elem in list(i.keys())) >= 7:
        if (len(i['byr']) == 4) and (1920 <= int(i['byr']) <= 2002) and (len(i['iyr']) == 4) and(2010 <= int(i['iyr']) <= 2020) and (len(i['eyr']) == 4) and (2020 <= int(i['eyr']) <= 2030) and (i['hcl'][0] == '#') and (len(i['hcl']) == 7) and (i['hcl'][1:].isalnum()) and (i['ecl'] in eye_color) and (len(i['pid']) == 9) and (i['pid'].isnumeric()) and (i['hgt'][-2:] == 'cm' or i['hgt'][-2:] == 'in'):
            if i['hgt'][-2:] == 'cm':
                if 150 <= int(re.sub('[^0-9]','',i['hgt'])) <= 193:
                    valid_count += 1
            elif i['hgt'][-2:] == 'in':    
                if 59 <= int(re.sub('[^0-9]','',i['hgt'])) <= 76:
                    valid_count += 1
